...
invoke(F&& f, As&&... as) RETURN(lib::forward<F>(f)(lib::forward<As>(as)...))
#ifdef _MSC_VER
#pragma warning(pop)
#endif
...
private:
  ...
  at(const lib::array<T, N>& elems, std::size_t i, Is... is) {
  ...
  inline static constexpr int visit_visitor_return_type_check() {
    static_assert(lib::all<std::is_same<F, Fs>::value...>::value,
                  "`jakalib::visit` requires the visitor to have a single "
                  "return type.");
    return 0;
  }

  template <typename... Fs>
  inline static constexpr lib::array<lib::common_type_t<lib::decay_t<Fs>...>,
                                     sizeof...(Fs)>
  make_farray(Fs&&... fs) {
    using result =
        lib::array<lib::common_type_t<lib::decay_t<Fs>...>, sizeof...(Fs)>;
  ...
  template <typename F, typename... Vs, std::size_t... Is>
  inline static constexpr auto
  make_fmatrix_impl(lib::index_sequence<Is...> is) {
  ...
  struct make_fmatrix_impl {
    ...
    struct impl<lib::index_sequence<Is...>, lib::index_sequence<Js...>, Ls...> {
      inline constexpr AUTO operator()() const AUTO_RETURN(
          make_farray(impl<lib::index_sequence<Is..., Js>, Ls...>{}()...))
    };
  };
...
public:
  ...
  visit_alt_at(std::size_t index, Visitor&& visitor, Vs&&... vs)
      DECLTYPE_AUTO_RETURN(
          ...
          template <typename Visitor, typename... Vs>
          inline static constexpr DECLTYPE_AUTO
      visit_alt(Visitor&& visitor, Vs&&... vs) DECLTYPE_AUTO_RETURN(at(
          make_fmatrix<Visitor&&, decltype(as_base(lib::forward<Vs>(vs)))...>(),
...
private:
  ...
  struct visit_exhaustive_visitor_check {
    static_assert(lib::is_invocable<Visitor, Values...>::value,
                  "`jakalib::visit` requires the visitor to be exhaustive.");
...
#pragma warning(pop)
  ...
  struct value_visitor {
    ...
    inline constexpr DECLTYPE_AUTO operator()(Alts&&... alts) const
        DECLTYPE_AUTO_RETURN(visit_exhaustive_visitor_check<
                             Visitor,
...
template <typename... Ts>
struct hash<jakalib::detail::enabled_type<
    jakalib::variant<Ts...>,
    jakalib::lib::enable_if_t<
        jakalib::lib::all<jakalib::detail::hash::is_enabled<
            jakalib::lib::remove_const_t<Ts>>()...>::value>>> {
  using argument_type = jakalib::variant<Ts...>;
  using result_type = std::size_t;

  inline result_type operator()(const argument_type& v) const {
    using jakalib::detail::visitation::variant;
...
#ifdef MPARK_GENERIC_LAMBDAS
            [](const auto& alt) {
              using alt_type = jakalib::lib::decay_t<decltype(alt)>;
              using value_type =
                  jakalib::lib::remove_const_t<typename alt_type::value_type>;
...
private:
  ...
  struct hasher {
    ...
    inline std::size_t operator()(const Alt& alt) const {
      using alt_type = jakalib::lib::decay_t<Alt>;
      using value_type =
          jakalib::lib::remove_const_t<typename alt_type::value_type>;
      return hash<value_type>{}(alt.value);
    }
...
template <>
struct hash<jakalib::monostate> {
  using argument_type = jakalib::monostate;
  using result_type = std::size_t;
